# Helper functions
# last version of Julia does not have support for the
...
@inline function solRefHelmholtz(x::Array{Float64,1},y::Array{Float64,1},z::Array{Float64,1}, sigma::Float64, k::Float64)
  r  = sqrt( x.^2 + y.^2 + z.^2)
  u  = (exp.(-sigma^2*k^2/2)./(4*pi.*r)).*(real( exp.(-1im.*k.*r).*erf( (2*sigma^2*1im*k - 2.0*r)./(2*sqrt(2*sigma^2) ) )) - 1im*sin(k.*r))
  return u
end

"""
 ref:https://arxiv.org/pdf/1604.03155.pdf
"""
@inline function Gtruncated2D(L::Float64,k::Float64,s::Array{T,N}) where {T<:Number, N}
...
"""
 ref:https://arxiv.org/pdf/1604.03155.pdf
"""
@inline function Gtruncated3D(L::Float64,k::Float64,s::Array{T,N}) where {T<:Number,N}
    return (-1 .+ exp.(1im*L*k).*( cos(L*s) .- (1im*k./s*sin(L*s)) ) )./(k^2  .- s.^2)
end
...
struct changeVariable
    ...
    function changeVariable(delta::Float64)
